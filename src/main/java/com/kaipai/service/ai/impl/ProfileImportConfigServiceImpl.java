...
public class ProfileImportConfigServiceImpl implements ProfileImportConfigService {
    ...
    public ProfileImportConfigRespDTO savePublicConfig(Long op, ProfileImportPublicConfigUpdateDTO d) {
        validateEndpoint(d.getEndpoint()); AiProfileImportConfig c = config();
        c.setEndpoint(d.getEndpoint()); c.setModelName(d.getModelName()); c.setConnectTimeoutMs(d.getConnectTimeoutMs());
        c.setReadTimeoutMs(d.getReadTimeoutMs()); c.setMaxInputChars(d.getMaxInputChars());
        c.setMaxOutputTokens(d.getMaxOutputTokens()); c.setPerUserDailyLimit(d.getPerUserDailyLimit());
        reset(c); persist(c); audit(op, c, "public_config_update", "success"); return dto(c);
    }
    public ProfileImportConfigRespDTO saveSecret(Long op, ProfileImportSecretUpdateDTO d) {
        if (d == null || !StringUtils.hasText(d.apiKey())) throw new BizException("DeepSeek API Key 不能为空");
        ...
        c.setSecretMaskJson(mask(d.apiKey())); reset(c); persist(c); audit(op, c, "secret_update", "success"); return dto(c);
    }
    public ProfileImportConfigRespDTO testConnection(Long op) {
        AiProfileImportConfig c = config(); if (!StringUtils.hasText(c.getSecretConfigCiphertext())) throw new BizException("请先配置密钥");
        ...
        c.setLastTestAt(LocalDateTime.now()); persist(c); audit(op, c, "test", c.getLastTestStatus()); return dto(c);
    }
    public ProfileImportConfigRespDTO setEnabled(Long op, boolean enabled) {
        AiProfileImportConfig c = config(); if (enabled && !ready(c)) throw new BizException("配置未通过连接测试");
        c.setEnabled(enabled); persist(c); audit(op, c, enabled ? "enable" : "disable", "success"); return dto(c);
    }
    public ProfileImportCapabilityRespDTO capability() { AiProfileImportConfig c = config(); boolean ok = Boolean.TRUE.equals(c.getEnabled()) && ready(c); return new ProfileImportCapabilityRespDTO(ok, ok ? null : "智能导入未启用"); }
    public ProfileImportRuntimeConfig runtimeConfig() {
        AiProfileImportConfig c = config();
        if (!Boolean.TRUE.equals(c.getEnabled()) || !ready(c)) throw com.kaipai.model.actor.dto.ProfileDomainErrorCode.PROFILE_IMPORT_DISABLED.toException();
        try { String key = json.readTree(crypto.decrypt(c.getSecretConfigCiphertext())).path("apiKey").asText(); return new ProfileImportRuntimeConfig(c.getConfigId(), c.getEndpoint(), c.getModelName(), key, c.getMaxInputChars(), c.getPerUserDailyLimit()); }
        catch (Exception error) { throw com.kaipai.model.actor.dto.ProfileDomainErrorCode.PROFILE_IMPORT_UNAVAILABLE.toException(); }
    }
