...
public class ActorMediaAssetServiceImpl implements ActorMediaAssetService {
    ...
    @Transactional(propagation=Propagation.MANDATORY, rollbackFor=Exception.class) public void requireOwnedReadyPhoto(Long userId,Long assetId){ requireOwnedReady(userId,assetId,"photo"); }
    @Transactional(propagation=Propagation.MANDATORY, rollbackFor=Exception.class) public void requireOwnedReadyPdf(Long userId,Long assetId){ requireOwnedReady(userId,assetId,"pdf"); }
    /** 归属 + 类型 + ready 三重校验。不存在/非本人一律 NOT_FOUND,不泄漏他人素材是否存在。 */
    private void requireOwnedReady(Long userId,Long assetId,String expectedMediaType){ ActorMediaAsset a=requireForUpdate(userId,assetId); if(!expectedMediaType.equals(a.getMediaType())||!"ready".equals(a.getProcessStatus())) throw ProfileDomainErrorCode.PROFILE_ASSET_NOT_READY.toException(); }
    ...
    public List<ActorAssetPageRespDTO> listPages(Long userId,Long assetId){ ActorMediaAsset a=require(userId,assetId); if(!"pdf".equals(a.getMediaType())) throw ProfileDomainErrorCode.PROFILE_ASSET_NOT_READY.toException(); if(!"ready".equals(a.getProcessStatus())) throw ProfileDomainErrorCode.PROFILE_ASSET_NOT_READY.toException(); List<ActorMediaAssetPage> pages=pageMapper.selectList(new LambdaQueryWrapper<ActorMediaAssetPage>().eq(ActorMediaAssetPage::getAssetId,assetId).orderByAsc(ActorMediaAssetPage::getPageNo)); return pages.stream().map(page->{ var signed=storage.issueAccessUrl(a.getBucketCode(),page.getImageObjectKey(),Duration.ofMinutes(10)); return new ActorAssetPageRespDTO(page.getPageNo(),signed.accessUrl(),signed.expiresAt()); }).toList(); }
